...
function compareIngredientsCB(ingredientA, ingredientB){
    ...
    // Aisle is the same, compare by name
...
function dishType(dish){
    // dishTypes property not present
    if (!dish.dishTypes) {
        return "";
    }
...
function sortDishes(dishes){
    return [...dishes].sort(compareDishTypeCB);
}
...
function menuPrice(dishesArray){
    ...
    return dishesArray.map(keepJustPriceCB).reduce(sumPriceOfDishesCB, 0);
